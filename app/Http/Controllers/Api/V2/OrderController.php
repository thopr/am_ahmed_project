...
class OrderController extends Controller
{
    ...
    public function complete_tap_payment(Request $request)
    {
        ...
        $orders = Order::where('combined_order_id', $request->order_id)->get();
        if(count($orders) > 0){
        ...
        return response()->json([         
            'result' => true,           
            'message' => translate('Your order has been paid successfully')
        ]); 
            ...
            return response()->json([         
                'result' => false,           
                'message' => translate('Not an order')
            ]);
        }
    ...
    public function store(Request $request, $set_paid = false)
    {
        ...
        //Storage::disk('local')->put('request2.txt', $request);
        ...
        if ($address != null) {
            ...
            if ($address->latitude || $address->longitude) {
                ...
                $shippingAddress['addresstext'] = $request->addresstext;
            }
        }
        ...
        foreach ($seller_products as $seller_product) {
            ...
            $order->dilivery_date = date('Y-m-d',strtotime($request->dilivery_date));
            
            

            $order->dilivery_time = $request->dilivery_time;
            $order->call = $request->call;
            $order->address_link = 'https://maps.google.com/?q='.$address->latitude.','. $address->longitude.'';
            $order->address_description = $request->address_description;            
            $order->addresstext = $request->addresstext; 
            $order->city = $shippingAddress['city']; 
            if($request->zonetext!=null){
                $order->zone = $request->zonetext; 
            }else{
         
           $last_order = Order::where('user_id', $user->id)->orderBy('created_at', 'desc')->first();

           if($last_order->zone!=null){
            $order->zone = $last_order->zone; 
            ...
            $order->code = date('Ymd') . rand(10, 99);
