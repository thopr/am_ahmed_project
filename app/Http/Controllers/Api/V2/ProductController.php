...
class ProductController extends Controller
{
    ...
    public function todaysDeal(Request $request)
    {
        ...
        $city = City::find($request->filter_city_id);
        if($request->filter_category_id){
            $main_category_id=$request->filter_category_id;
        }
        $products = $city->products()->where('todays_deal', 1)->where('main_category_id', $main_category_id)->physical();
        ...
        return Cache::remember('app.todays_deal_'.$request->filter_city_id."_".$request->filter_category_id."", 86400, function() use ($request) {
    ...
    public function featured(Request $request)
    {
        ...
        $products = $city->products()->where('featured', 1)->where('main_category_id', $main_category_id)->physical();
     
      //  $products = Product::where('featured', 1)->where('main_category_id', $main_category_id)->physical();
        return new ProductMiniCollection(filter_products($products)->latest()->paginate(10));
    }
    ...
    public function search(Request $request)
    {
        ...
        if($request->filter_city_id){
            ...
            $products = $city->products()->where('published', 1)->where('main_category_id', $main_category_id)->physical();
        }else{
         $products = Product::query();
         $products = $products->where('published', 1)->physical();
        }
