...
class CategoryController extends Controller
{
    ...
    public function index( Request $request, $parent_id = 1)
    {
        ...
        if($parent_id==0){
            ...
            if($request->filter_category_id){
                $parent_id=$request->filter_category_id;
    ...
    public function featured(Request $request)
    {
        ...
        if($request->filter_category_id){
            $parent_id=$request->filter_category_id;
            return Cache::remember('app.featured_categories_'.$request->filter_category_id, 86400, function() use ($parent_id){
            return new CategoryCollection(Category::where('featured', 1)->where('parent_id',$parent_id)->get());       
        });
        }else{
