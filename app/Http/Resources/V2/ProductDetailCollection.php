        ...
        return [
            'data' => $this->collection->map(function ($data) {
                ...
                return [
                    ...
                    'small_summary' => $data->small_summary,
...
$new_values = array();
            foreach($values as $value) {
                $temp_value_array=explode(':',$value);
               $name= $temp_value_array[0];
               $price=$temp_value_array[1];
               $price += ($price * 15) / 100;
               $temp_value_array[1]=round($price);
              $temp_name=$temp_value_array[0].':'.$temp_value_array[1];
               array_push($new_values,$temp_name);
            }
            $item['name'] = $choice->attribute_id;
            $item['title'] = Attribute::find($choice->attribute_id)->getTranslation('name');
            $item['options'] = $new_values;
