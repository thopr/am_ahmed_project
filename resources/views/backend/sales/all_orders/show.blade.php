...
<div class="card">
    ...
    <div class="card-body">
        <div class="row gutters-5">
            ...
            <div class="col-md-3 ml-auto">
                <label for="update_delivery_status">{{translate('Assign Delivery')}}</label>               
                    <select class="form-control aiz-selectpicker"  data-minimum-results-for-search="Infinity" id="update_delivery_person">
                           @php
                            $role=\App\Models\Role::where('name','Delivery')->first();
                            if($role){
                                $staffs=\App\Models\Staff::where('role_id',$role->id)->with('user')->get();
                            }else{
                                $staffs=null;
                            }
                            ...
                            <option value={{$staff->user_id}} @if ($order->dilivery_user_id == $staff->user_id) selected @endif>{{$staff->user->name}}</option>
                            @endforeach
            ...
            <div class="col-md-3 ml-auto ">
                <form method="post" action={{ route('orders.notify_delivery_person') }}>     
                    @csrf
                    <input type="hidden" name="order_id" value={{$order->id}} >                     
                <button  class="btn btn-small btn-danger mt-4" type="submit"  value="notify_delivery_person">{{translate('Notify Delivery Person')}}</button>
                </form>
                <form style="d-none" method="post" action={{ route('orders.send_qr_invoice') }}>     
                    @csrf
                    <input type="hidden" name="order_id" value={{$order->id}} >                     
                <button  class="btn btn-small btn-danger mt-4" type="submit"  value="send_qr_invoice">{{translate('Send invoice to user')}}</button>
                </form>
            </div>
            ...
            <div class="col-md-4 ml-auto">
                <table>
                    <tbody>
                        ...
                        <tr>
                            <td class="text-main text-bold">{{translate('Dilivery Date')}}</td>
                            <td class="text-right">{{$order->dilivery_date}}</td>
                        </tr>
                        <tr>
                            <td class="text-main text-bold">{{translate('Dilivery Time')}}</td>
                            <td class="text-right">{{$order->dilivery_time == 'pm' ? translate('evening') : translate('morning') }}</td>
                        </tr>
                        <tr>
                            <td class="text-main text-bold">{{translate('Call Before Dilivery')}}</td>
                            <td class="text-right">{{$order->call ==1 ? translate('Yes') : translate('No') }}</td>
                        </tr>
                        <tr>
                            <td class="text-main text-bold">{{translate('Dilivery Address')}}</td>
                            <td class="text-right"><a target="_blank" href={{$order->address_link}} class="text-right"> <span class="badge badge-inline badge-success">Link</span></a></td>
                        </tr>

                        <tr>
                            <td class="text-main text-bold">{{translate('Address')}}</td>
                            <td class="text-right">{{$order->address_description}}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <p @if(false) href="{{ route('product', $orderDetail->product->slug) }}" target="_blank" @endif><img height="50" src="{{ uploaded_asset($orderDetail->product->thumbnail_img) }}"></p>
                            ...
                            <td>
                                @if ($orderDetail->product != null && $orderDetail->product->auction_product == 0)
                                <strong> <p @if(false) href="{{ route('product', $orderDetail->product->slug) }}" target="_blank" @endif class="text-muted">{{ $orderDetail->product->getTranslation('name') }}</p></strong>
...
@section('script')
    <script type="text/javascript">
        ...
        $('#update_delivery_person').on('change', function(){
            var order_id = {{ $order->id }};
            var user_id = $('#update_delivery_person').val();
            $.post('{{ route('orders.update_delivery_person') }}', {_token:'{{ @csrf_token() }}',order_id:order_id,user_id:user_id}, function(data){
                AIZ.plugins.notify('success', '{{ translate('Delivery Person has been updated') }}');
            });
        });
