                        ...
                        <tbody>
                            @foreach($states as $key => $state)
                                <tr>
                                    ...
                                    <td>
                                        <img src="{{ uploaded_asset($state->image) }}" alt="{{translate('state')}}"
                                         height="50" width="50" 
                                        class="h-50px"
                                    ...
                                    <label class="aiz-switch aiz-switch-success mb-0">
                                        <input onchange="update_featured(this)" value="{{ $state->id }}" type="checkbox" <?php if($state->featured == 1) echo "checked";?> >
                                        <span class="slider round"></span>
                                    </label>
                                </td>
                                                                    <td class="text-right">
    			...
    			<div class="card-body">
    				<form action="{{ route('states.store') }}" method="POST">
                        ...
                        <div class="form-group mb-3">
    						...
    						<input type="text" placeholder="{{translate('latitude')}}" name="latitude" class="form-control" required>
    					</div>
                        <div class="form-group mb-3">
    						<label for="name">{{translate('Longitude')}}</label>
    						<input type="text" placeholder="{{translate('longitude')}}" name="longitude" class="form-control" required>
    					</div>
                        <div class="form-group mb-3">
    						<label for="name">{{translate('Summary')}}</label>
    						<input type="text" placeholder="{{translate('summary')}}" name="summary" class="form-control" required>
    					</div>


    
                        <div class="form-group mb-3">
                            <label for="name">{{translate('Image')}} <small>({{ translate('120x80') }})</small></label>
...
function update_featured(el){
            ...
            $.post('{{ route('states.featured') }}', {_token:'{{ csrf_token() }}', id:el.value, featured:featured}, function(data){
                if(data == 1){
                    AIZ.plugins.notify('success', '{{ translate('Country featured updated successfully') }}');
                }
                else{
