...
@section('content')
    ...
    <div class="row">
    	<div class="col-lg-8 mx-auto">
			...
			<div class="card">
    			<div class="card-header">
    				<h6 class="fw-600 mb-0">{{ translate('Extra App Settings') }}</h6>
    			</div>
    			<div class="card-body">
    				<form action="{{ route('business_settings.update') }}" method="POST" enctype="multipart/form-data">
    					@csrf
    					<div class="form-group row">
    						<label class="col-md-3 col-from-label">{{ translate('Whatsapp Phone Number') }}</label>
                            <div class="col-md-8">
        						<input type="hidden" name="types[]" value="whatsapp_phone">
        						<input type="text" class="form-control" placeholder="{{translate('Whatsapp Phone Number')}}" name="whatsapp_phone" value="{{ get_setting('whatsapp_phone') }}">
                            </div>
    					</div>

						<div class="form-group row">
    						<label class="col-md-3 col-from-label">{{ translate('Delivery time') }}</label>
                            <div class="col-md-8">
        						<input type="hidden" name="types[]" value="delivery_time">
        						<input type="text" class="form-control" placeholder="{{translate('Delivery time')}}" name="delivery_time" value="{{ get_setting('delivery_time') }}">
                            </div>
    					</div>
