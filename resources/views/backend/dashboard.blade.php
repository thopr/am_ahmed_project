...
<div class="card">
    <div class="card-header">
        <h6 class="mb-0">{{ translate('Orders Map') }}</h6>
    </div>
    <div class="card-body">
        @php
            $orders_locations =array();
            $orders = \App\Models\Order::where('delivery_status','!=','delivered')->get();

            foreach($orders as $order){
                $shipping_address = json_decode($order->shipping_address, true);
                if(defined($shipping_address['lat_lang'])){
                    $lat_long = $shipping_address['lat_lang'];
                    ...
                    array_push($orders_locations, $point);
...
<style>
    ...
    function initialize() {
      ...
      const map = new google.maps.Map(document.getElementById("mapf"), {
        zoom: 3,
      ...
      var myElement = document.getElementById("mapf");
    ...
    const locations = {!!$orders_locations!!};
